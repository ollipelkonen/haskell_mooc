...
firstHalf x = take (ceiling ((fromIntegral $ length x) / 2.0)) x
...
capitalize x = unwords $ map d (words x)
    where d x = (toUpper (head x)) : (tail x)
...
powers :: Int -> Int -> [Int]
powers k max = powers_ 0 k max []
powers_ n k max val
    | k^n > max = val
    | otherwise = powers_ (n+1) k max (val ++ [k^n])
...
joinToLength len x = [whole | first <- x,
                              last <- x,
...
sumRights values = sumRights_ values 0
sumRights_ (x:xs) total = case x of 
    Right n -> sumRights_ xs (total+n)
    Left _ -> sumRights_ xs total
sumRights_ [] total = total
...
interpreter_ :: [String] -> [Int] -> [String] -> [String]
interpreter_ [] pos output = output
interpreter_ (command:commands) pos output = interpreter_ commands (interpreter_move command pos) (interpreter_output command pos output)

interpreter_move :: String -> [Int] -> [Int]
interpreter_move command pos = case command of
    "up" -> head pos : [(+) (last pos) 1]
    "down" -> head pos : [(-) (last pos) 1]
    "right" -> ((+) (head pos) 1) : (tail pos)
    "left" -> ((-) (head pos) 1) : (tail pos)
    _ -> pos
interpreter_output :: String -> [Int] -> [String] -> [String]
interpreter_output command pos output = case command of
    "printX" -> output ++ [show (head pos)]
    "printY" -> output ++ [show (last pos)]
    _ -> output
