...
data State = Start | Eggs | Sugar | Flour | FlourSugar | Mixed | Error | Finished
  deriving (Eq,Show)
...
average (x:|xs) = (sum list) / fromIntegral (length list)
  where list = (x:xs)
...
show1 (Add1 i j) = (show i) ++ (id "+") ++ (show j)
show1 (Subtract1 i j) = (show i) ++ (id "-") ++ (show j)
show1 (Multiply1 i j) = (show i) ++ (id "*") ++ (show j)
...
instance Operation2 Add2 where
  show2 (Add2 i j) = (show i) ++ (id "+") ++ (show j)
  compute2 (Add2 i j) = i+j

instance Operation2 Subtract2 where
  show2 (Subtract2 i j) = (show i) ++ (id "-") ++ (show j)
  compute2 (Subtract2 i j) = i-j

instance Operation2 Multiply2 where
  show2 (Multiply2 i j) = (show i) ++ (id "*") ++ (show j)
...
passwordAllowed s (ContainsSome l) = foldr (\x y -> y || elem x s) False l
passwordAllowed s (DoesNotContain l) = not $ foldr (\x y -> y || elem x s) False l
passwordAllowed s (And l1 l2) = (passwordAllowed s l1) && (passwordAllowed s l2)
passwordAllowed s (Or l1 l2) = (passwordAllowed s l1) || (passwordAllowed s l2)
...
operation op l1 l2 = Operation op l1 l2
...
render (Operation "+" l1 l2) = (id "(") ++ (render l1) ++ (id "+") ++ (render l2) ++ (id ")")
render (Operation "*" l1 l2) = (id "(") ++ (render l1) ++ (id "*") ++ (render l2) ++ (id ")")
