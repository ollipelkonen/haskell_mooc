...
treeMax t = case t of
  Empty -> 0
  Node n a b -> max n $ max (treeMax a) (treeMax b)
...
allValues condition tree = case tree of
  Empty -> True
  Node n a b -> condition n && (allValues condition a) && (allValues condition b)
...
mapTree f t = case t of
  Empty -> Empty
  Node n a b -> Node (f n) (mapTree f a) (mapTree f b)
...
cull val tree = case tree of
  Empty -> Empty
  Node n a b -> if n == val then Empty else Node n (cull val a) (cull val b)
...
--   Empty -> True
--   Node n a b -> (toLeft n a) && (toRight n b)
--   where
--     toLeft n1 a = case a of
--       Empty -> True
--       Node n2 a2 b2 -> if n2>n1 then False else (toLeft n2 a2) && (toRight n2 b2)
--     toRight n1 a = case a of 
--       Empty -> True
--       Node n2 a2 b2 -> if n2<n1 then False else (toLeft n2 a2) && (toRight n2 b2)
...
isOrdered t = case t of
  Empty -> True
  Node n a b -> (follow (>) n a) && (follow (<) n b)
  where
    follow f n1 a = case a of
      Empty -> True
      Node n2 a2 b2 -> if (f n2 n1) then False else (follow (>) n2 a2) && (follow (<) n2 b2)
...
walk [] tree = case tree of
  Empty -> Nothing
  Node n _ _ -> Just n
walk (step:steps) tree = case step of
  StepL -> walkLeft steps tree
  StepR -> walkRight steps tree
  where
    walkLeft steps tree = case tree of
      Empty -> Nothing
      Node n a b -> walk steps a
    walkRight steps tree = case tree of
      Empty -> Nothing
      Node n a b -> walk steps b
...
set [] val tree = case tree of
  Empty -> Empty
  Node n a b -> Node val a b
set (step:steps) val tree = case step of
  StepL -> walkLeft steps tree
  StepR -> walkRight steps tree
  where
    walkLeft steps tree = case tree of
      Empty -> Empty
      Node n a b -> Node n (set steps val a) b
    walkRight steps tree = case tree of
      Empty -> Empty
      Node n a b -> Node n a (set steps val b)
...
search val tree = case tree of 
  Empty -> Nothing
  Node n a b -> if n == val then Just [] else search'' val tree
  where
    search'' val_ tree_ = case search' [] val_ tree_ of
      [] -> Nothing
      x -> Just x
      where 
        search' path val tree = case tree of
          Empty -> []
          Node n a b -> if n == val then path else (search' (path ++ [StepL]) val a) ++ (search' (path ++ [StepR]) val b)
