...
longest xs = foldr tst [] xs where
tst x y
    | lx == ly = if head x < head y then x else y
    | length x > length y = x
    | otherwise = y
    where
        lx = length x
...
incrementKey n xs = foldr (\x k -> if fst x == n then (n, 1 + snd x):k else x:k) [] xs 
...
freqs xs = Map.fromList $ zipWith (\n x-> (n, length(filter (==n) xs))) (nx) (nx)
    where nx = nub xs
...
transfer from to amount bank = if allowed then transaction else bank
    where
        allowed = Map.member from bank && Map.member to bank && amount > 0 && Map.findWithDefault 0 from bank >= amount
        transaction = Map.insertWith (\a b -> b-a) from amount (Map.insertWith (\a b -> b+a) to amount bank)
...
maxIndex xs = fst ( foldr (\x y -> if snd x > snd y then x else y) (xss !! 0) xss )
    where xss = Data.Array.assocs xs
